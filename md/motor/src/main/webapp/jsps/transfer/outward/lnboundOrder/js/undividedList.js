var TableInitUndividedList = function() {
	var oTableInitUndividedList = new Object();
	//初始化Table 
	oTableInitUndividedList.Init = function() {
		$('#tb_undividedList').bootstrapTable({
			method : 'get', //请求方式（*） 
			...
			sortOrder : "asc", //排序方式
			queryParams : oTableInitUndividedList.queryParams,
	...
	oTableInitUndividedList.queryParams = function(params) {
		var temp = {
	...
	oTableInitUndividedList.queryParamsWork = function(params) {
		var temp = {
			//这里的键的名字和控制器的变量名必须一直，这边改动，控制器也需要改成一样的 
			limit : params.limit, //页面大小 
			offset : params.offset //页码 
	...
	return oTableInitUndividedList;
};

var ButtonInitUndividedList = function() {
	var oInit = new Object();
	var postdata = {};
	oInit.Init = function() {
		$("#save").click(function(){
			var select = $('#tb_undividedList').bootstrapTable(
				'getSelections');
			var workOrderId = [];
...
$(function() {
	...
	//1.初始化Table 
	var oTableInitUndividedList = new TableInitUndividedList();
	oTableInitUndividedList.Init();
	//2.初始化Button的点击事件 
	var oButtonInitUndividedList = new ButtonInitUndividedList();
	oButtonInitUndividedList.Init();
	//3.初始化消息提示框
