var TableInitDiskData = function() {
	var oTableInitDiskData= new Object();
	// 初始化属性实盘数据Table
	oTableInitDiskData.Init = function() {
		$('#tb_diskData').bootstrapTable({
			url : '', // 请求后台的URL（*）
			method : 'get', // 请求方式（*）
			toolbar : '#toolbar_diskData', // 工具按钮用哪个容器
			striped : true, // 是否显示行间隔色
			cache : false, // 是否使用缓存，默认为true，所以一般情况下需要设置一下这个属性（*）
			pagination : true, // 是否显示分页（*）
			sortable : false, // 是否启用排序
			sortOrder : "asc", // 排序方式
			queryParams : oTableInitDiskData.queryParams,// 传递参数（*）
	...
	// 初始化工单咨询Table
	// 得到查询的参数
	oTableInitDiskData.queryParams = function(params) {
		var temp = {
	...
	return oTableInitDiskData;
};

var ButtonInitDiskData = function() {
