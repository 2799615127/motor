...
<body>
	...
	<div>
		<button button id="statisticalData" type="button"
			class="btn btn-default1" data-toggle="modal" data-target="#myModal">
			<span aria-hidden="true">统计数据</span>
		</button>
		<div id="statisticalDatas" style="display: none">
			<div>
				<input disabled="disabled" id="totalVotes" class="col-sm-2"
					style="margin-top: 10px"></input> <input disabled="disabled"
					id="inOrderToDeliver" class="col-sm-2" style="margin-top: 10px"></input>
				<input disabled="disabled" id="notDelivered" class="col-sm-2"
					style="margin-top: 10px"></input> <input disabled="disabled"
					id="delayRate" class="col-sm-2" style="margin-top: 10px"></input>
			</div>
		</div>
	</div>
	<div>
	...
	var TableInit = function() {
		...
		oTableInit.Init = function() {
			...
			$('#tb_departments').bootstrapTable({
				url : '/noob/appointController/appointPageQuery.action', //请求后台的URL（*）
				method : 'get', //请求方式（*）
					...
					.click(
							function() {
								if ($("#statisticalDatas").hasClass("show")) {
									$("#statisticalDatas").hide().removeClass(
											"show");
								} else {
									$
											.ajax({
												type : "GET",
												url : '/noob/appointController/statisticalDataPageQuery.action',
												data : {},
							...
							function() {
								...
								if (select.length == 0) {
									...
									$
											.ajax({
												type : "GET",
												url : '/noob/appointController/appointJobListIdsQuery.action',
												data : {
													jobListIds : arrs
												},
												success : function(result) {
